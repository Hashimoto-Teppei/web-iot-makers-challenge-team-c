...
走行前後に検知ログ・走行ログを確認するためのアプリ。Expo（React Native）で作る。
**iOS が主ターゲットで、Android も残す**（`../../docs/adr/0010-ios-primary-target.md`）。

**走行中にこの画面を見る前提の機能を作らないこと。** 走行中のスマホ操作は取り締まりの対象で、
危険の通知はデバイス側（LED / ディスプレイ）で完結させる方針です。
...
（位置情報は個人情報である。`AGENTS.md`）。**手元に向けているときだけ実際に中継します。**
...
- **iOS（主）** — **TestFlight で配ります**（`../../docs/adr/0010-ios-primary-target.md`）。
  ...
  **手順は [`../../docs/deploy-mobile.md`](../../docs/deploy-mobile.md) にあります。**
