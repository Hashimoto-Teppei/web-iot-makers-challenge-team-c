 ...
 * （見る行為が取り締まりの対象。`AGENTS.md`）ので、**走り出す前に人が1画面で見ることが
 * 実質唯一の防御**になる。
 *
...
export type PreRideCheck = {
   ...
   *
   * 「デバイス: ✗」だけだと、初めての人は手が止まる（`AGENTS.md`
   * 「開発が初めてのメンバーが多い」）。
   */
  detail: string;
};
...
export type PreRideInput = {
   ...
   * 「デバイス: ✗」だけだと初めての人は手が止まる（`AGENTS.md`）。
   */
  deviceReason: string | null;
  /** まだ探している最中か。**「探している」と「駄目だった」を混ぜない** */
  deviceChecking: boolean;
  /**
   * 「デバイスを使わない」がオンか（#185。`./standalone.ts`）。
   *
   * **オンなら、つながっていなくても緑にする** —— デモの2台目のスマホは
   * 中継だけを担うので、デバイスが無いのが正しい状態である。
   * **ただし「この端末には警告が出ない」ことを必ず出す**（隠すと、この画面が
   * 防ごうとしている「動いているつもり」そのものになる）。
   */
  standalone: boolean;
  /**
...
function device({
  ...
  if (deviceId === null) {
    ...
    if (deviceReason === null && deviceChecking) {
      // **待てば直ることを、この行に書いておく。**別のスマホが先につながっていると
      // **見つかっても切られる**（デバイスは先着優先。#184）が、
      // その先着も心拍を書かなければ 30 秒ほどで手放される
  ...
  // **走行中だけ見る。**走り出す前も心拍は出ている（`./idle-heartbeat.ts`。#128）が、
  // **つないでから `link` が上がるまでに数秒かかる**ので、**走り出す前の `down` は
  // まだ正常な状態**である。ここで赤くすると、**つないだ直後がいつも赤に見え**、
  // 本物の赤が読み飛ばされるようになる（`../ble/link.ts` の偽の ✗ と同じ話）。
  //
  // **走行前に `down` のまま留まることは、もう「正常」ではない**——心拍が出ているのに
  // 届いていないことになる。ただし**待てば直るものと区別が付かない**うちは赤にしない。
  // どこで諦めるかを決めるなら別途（#128 の実装後に見えてくる）。
  //
