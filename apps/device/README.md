...
（#35）。**それを実際に光らせる・表示する `hw/` 側がまだありません**
（**部品は #13 で決まりました** —— LCD1602A / LED ×2。
ピンとアドレスは `src/device/config.py` の末尾、配線は `docs/hardware.md`）。`notify.py` は値を返すだけで、ハードウェアに触りません。
繋がっていることは、`warn` を書いたときに journalctl へ出る `表示:` の行で確かめます。
