...
言語もテストの仕組みも違うので、共有できるものはありません（`AGENTS.md`「`packages/` はまだ作らない」）。
...
出すのは `hw/` 側で、**何を出すかを決め直しません**（この境界を壊さないでください）。

**部品が載っているかどうかは `src/device/config.py` の末尾の値で決まります。**
ピンやアドレスを `None` にすると、その部品は**無いものとして動きます**——
**実機も部品も持っていない人は、4つとも `None` にすれば `hw/` で止まりません。**
配線の正本は `../../docs/hardware.md` です。

出したものは journalctl にも `表示:` の行として残ります（**走行中に読めるのはそこだけ**です）。

**起動直後、警告の LED が1秒だけ光ります。** `link` の LED は `down` から始まるので勝手に
点滅しますが、警告の LED は警告が来るまで消灯のままで、**切れていても走り出すまで
分からない**ためです（`../../docs/notifications/arbitration.md`）。
