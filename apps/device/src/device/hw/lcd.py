...
class Lcd:
    ...
    **下段は毎秒変わる**（生存のコマが進むため。`../notify.py` の `link_frames`）。
    **上段は警告が変わるまで書き直されない**ので、ここで減るのは上段のぶんである。
    **毎秒の書き直しでちらつかないかは実機で見る**（`../../../../../docs/unverified.md`）。
    """
    ...
    **`None` で落ちないことが要件である**——実機も部品も持たない開発者が、
    ...
    try:
        display = CharLCD(
            ...
            port=1,
            # **内蔵フォントの ROM。指定しないと RPLCD の既定は `A02` で、半角カタカナが
            # 載っていない方に当たる**（あちらの `CharLCD.__init__`。1.4.0 で確認）。
            # **ASCII の範囲は A00 と A02 でほぼ同じなので、英字だけのうちは間違いが表に出ない**
            # ——カナを出した瞬間に別の字が出る。
            # **実物が A00 であることは 2026-09-20 に実機で確かめた**（`ｱｲｳｴｵ` がそのまま出た）。
            charmap="A00",
            cols=LCD_COLUMNS,
            rows=LCD_ROWS,
