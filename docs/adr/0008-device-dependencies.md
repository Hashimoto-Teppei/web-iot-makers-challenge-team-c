...
連鎖に入る `colorzero` と `setuptools` も同じなので、**ARMv6 でソースビルドに落ちない。**

**ピンファクトリを足していない。** gpiozero は `lgpio` → `RPi.GPIO` → `pigpio` →
純 Python の `native` の順に探し、**前の3つはどれも C 拡張**である。
**何も足さず `native` に落ちる前提**で進めており、外れたら実機に1行足す（`../unverified.md` 99）。
