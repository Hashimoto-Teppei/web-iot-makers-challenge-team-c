...
ハッカソン期間内に、デバイス（Raspberry Pi Zero 2 W）・モバイル・API・Web の4コンポーネントを複数人で並行開発する。
...
**版は「実機のシステム Python に合わせる」で決めており、実機を見て2回動かしている。**
3.12 → 3.11（#52）→ **3.13（2026-09-19）**。**Raspberry Pi OS のシステム Python は
Bookworm が 3.11、Trixie が 3.13 で、3.12 はどちらにも入っていない**——この事情は変わっていない。

**3.13 へ上げたのは、実機が Zero 2 W（aarch64）で Trixie が載っていたから**
（`../unverified.md` 40。実測: Python 3.13.5）。**Zero W（ARMv6）を前提に 3.11 を選んだ理由は、
機種が変わった時点で消えた。**

- **「Trixie で ARMv6 が切られるので Bookworm が最終」という理由**は、**aarch64 では成り立たない**
- **「uv が実機に Python を用意できない」という制約も消えた。** python-build-standalone は
  armv7 以上にビルドを出しており、**aarch64 には有る。**
  それでもシステム Python に合わせるのは、**`PyGObject` が要る共有ライブラリを apt 側と
  噛み合わせるため**である（`0008-device-dependencies.md`）

デバイスが使うのは標準ライブラリと BLE / GPIO のライブラリだけで、版ごとの新機能に依存していない。
**次に実機の OS が変わったら、ここもまた動かす。**
...
**追記（2026-09-03）: Workers Paid（$5/月）へ移行した。** 上の「無料枠で常時稼働できる」は
**選定した当時の理由**であり、いまの前提ではない。**採用そのものは変えない**（移行後も
Workers・D1 の組み合わせが最も手数が少ないため）。**プランの正本は
[`apps/web/README.md`](../../apps/web/README.md)「デプロイ担当だけがやること」。**
...
> **主ターゲットは `0010-ios-primary-target.md` で iOS に変更した**（2026-09-14）。
> **変えたのは対象 OS だけで、下の判断のうち Expo・ローカルビルド（EAS を使わない）は
> そのまま生きている。**以下は「なぜ Android を主に選んだか」の記録として残す
> ——理由は機能ではなく配布（7日で失効する署名）であり、**その制約が外れたことが
> `0010` の中身である。**
...
具体的な運用ルールは `AGENTS.md` の「機密情報の扱い」を参照。
