...
- 関連: `0001-tech-stack.md`（Android を主と決めた。**ここで変更する**）/
  ...
  `../deploy-mobile.md`（配る手順。**まだ無い** → #162）
...
- **外部グループへの1本目は Beta App Review を通る。**待ち時間が読めないので、
  **中身が未完成でも最初に1本上げて審査に入れる**（手順は #162 で `../deploy-mobile.md` に書く）。
...
iOS のビルドは Mac + Xcode が要るため、`CLAUDE.md`「どちらでも同じ手順で開発できることを
...
**個人の割り当てはここに書かない**（`CLAUDE.md`「担当者をドキュメントに書かない」）。
...
**`CLAUDE.md` の例外「静かに黙る故障」に当たるかを確かめたが、当たらない**
...
- **`0001` の「ストアに提出せず端末へ直接インストールする前提」が変わる。**
  ...
  かかるのはこのためである。**
- **`../deploy-mobile.md` を新設する**（`../deploy-device.md` と対になる）。**まだ無い** → #162。
  **実際に1本配ってから書く**——配る前に書いた手順は、書いた人以外に検証できない。
  `../setup.md` は 384 行あり、配布手順を足すと `CLAUDE.md` の 400 行に達する。
...
`../unverified.md` の 95〜98（iOS の背景動作 / MTU / `304` の素通り / GATT のキャッシュ）。
