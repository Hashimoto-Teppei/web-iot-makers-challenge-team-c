...
- **`apps/mobile`（Android）** — Android Studio と JDK 17、そして数時間かかる初回ビルドが必要です。
  ...
  **書き換えたら Metro を `--clear` 付きで再起動してください**（`npx expo start --clear`）。
  ...
  >
  > **これはコードでも止めてあります**（`apps/mobile/src/lib/mock-guard.ts` の `blocksMockDevice`）。
  > 既定のまま開始すると中継は毎回失敗し、画面に失敗の回数が出ます——**壊れているのではなく、
  > 手元に向けろという合図**です。検知そのものはネットワーク無しで
  > シミュレータから確かめられます（`apps/mobile/src/sim/`）。

  **標識を作り直したときは、端末の中の `signs.db` を消してください。**
  同梱物は**端末に `signs.db` が無いときだけ**コピーされるため、上書きインストールでは
  古いままになります（設定画面には古い版が「そろっている」顔で出ます）。

  ```sh
  adb shell run-as jp.teamc.bikealert rm -f files/SQLite/signs.db files/SQLite/signs.db-journal
  ```

  次の起動で新しい同梱物がコピーされます。**走行ログ（`app.db`）は消えません。**
  `adb` は Android Studio に入っています。**`run-as` は Development Build（debug）でしか
  使えない**ので、release ビルドを入れている端末では**アプリをアンインストールして**
  入れ直してください（走行ログは消えます）。

  **アプリは起動時に、向いている API の版へ標識を合わせます**
  （`docs/interfaces/stop-signs-delivery.md`「版はサーバーが決める」）。**上の向け先が食い違っていると、
  手元から作った同梱物が、次の起動でデプロイ先のものに入れ替わります。**
  入れ替えたかどうかは設定画面の「更新」に出ます。
...
| 同梱物を入れたのに「一時停止の標識を持っていません」と出る | 端末の中の `signs.db` が古い形式（`meta` に外接矩形の列が無い。#72）。`signs:build` で作り直したうえで、上の「標識を作り直したとき」の手順で入れ替える |
| 標識を作り直したのにアプリの件数・版が変わらない | 上書きインストールでは同梱物が入れ替わらない。上の「標識を作り直したとき」の手順で端末の `signs.db` を消す |
