...
- **リポジトリの状態を外から見える形で変えるコマンドは、人が指示したときだけ実行する**。
  ...
  AI エージェント（Claude Code）は差分を作るところまでで止め、**実行の可否は人が決める**。
...
- **`/code-review` はコミットの前に回す**。指摘の修正がそのままコミットに入り、レビューが1周で終わる。
  コミットしてから回すと、直すたびにコミットが増えて何周もすることになる。
  作業中のこまめなコミットのたびに回す必要はない。**人に見せる形になったときに1回**でよい。
